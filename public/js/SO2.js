document.addEventListener("DOMContentLoaded", async () => {
    ...
    const canvas = document.getElementById('so2Chart');
    if (!canvas) {
    ...
    const fetchData = async () => {
        try {
            ...
            if (!pollutantData.so2 || pollutantData.so2.every(value => value === null)) {
                console.error("SO₂ verisi eksik veya bulunamadı!");
                alert("Seçilen istasyona ait SO₂ verisi bulunamadı!");
    ...
    if (!pollutantData.so2 || pollutantData.so2.length === 0) {
        console.error("SO₂ verisi eksik veya yüklenemedi!");
        return;
    }

    const threshold = 35; // SO₂ sınır değeri sabit (µg/m³)
    ...
    const createChart = () => {
        const tooltipData = pollutantData.so2.map(value => 
        ...
        return new Chart(ctx, {
            type: 'line',
            data: {
                labels: monthLabels, // Ay isimlerini ekledik
                datasets: [
                    { // SO₂ Gerçek Değerler
                        label: 'SO₂ (Gerçek Değerler)',
                        data: pollutantData.so2.map(value => value !== null ? value : 0),
                        borderColor: 'rgba(153, 102, 255, 1)',
                        backgroundColor: 'rgba(153, 102, 255, 0.2)',
                        borderWidth: 2,
                        fill: true,
                        pointBackgroundColor: pollutantData.so2.map(value => value > threshold ? 'red' : 'blue'),
                        pointRadius: 5
                    },
                    { // SO₂ Sınır Değeri
                        label: `SO₂ Sınır (${threshold} µg/m³)`
,
                    ...
                    { // SO₂ Hedef Değeri
                        label: `SO₂ Hedef (%${targetPercentage})`,
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'SO₂ Değerleri: Sınır ve Hedef'
                    },
                    tooltip: {
                        callbacks: {
    ...
    calculateButton.addEventListener('click', () => {
        ...
        const reducedSO2 = pollutantData.so2.map(value => value !== null ? value - (value * reductionPercentage) : 0);

        // Daha önce eklenen azaltılmış veri setini kaldır
        const datasetIndex = chart.data.datasets.findIndex(dataset => dataset.label.includes('SO2 Azaltılmış'));
        if (datasetIndex !== -1) {
        ...
        chart.data.datasets.push({
            label: `SO2 Azaltılmış (%${reductionPercentage * 100})`,
            data: reducedSO2,
            borderColor: 'blue',
            borderDash: [4, 4],
            borderWidth: 2,
