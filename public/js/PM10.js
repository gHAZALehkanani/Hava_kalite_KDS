document.addEventListener("DOMContentLoaded", async () => {
    ...
    const fetchData = async () => {
        try {
            ...
            if (!pollutantData.pm10 || pollutantData.pm10.every(value => value === null)) {
                console.error("PM10 verisi eksik veya bulunamadı!");
                alert("Seçilen istasyona ait PM10 verisi bulunamadı!");
    ...
    if (!pollutantData || pollutantData.length === 0) {
        console.error("PM10 verisi eksik veya yüklenemedi!");
        return;
    }

    const threshold = 12; // PM10 sınır değeri sabit (µg/m³)
    ...
    const createChart = () => {
        const tooltipData = pollutantData.pm10.map(value => 
        ...
        return new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [
                    { // PM10 Gerçek Değerler
                        label: 'PM10 (Gerçek Değerler)',
                        data: pollutantData.pm10.map(value => value !== null ? value : 0),
                        borderColor: 'rgba(75, 192, 192, 1)',
                        backgroundColor: 'rgba(75, 192, 192, 0.2)',
                        borderWidth: 2,
                        fill: true,
                        pointBackgroundColor: pollutantData.pm10.map(value => value > threshold ? 'red' : 'blue'),
                        pointRadius: 5
                    },
                    { // PM10 Sınır Değeri
                        label: `PM10 Sınır (${threshold} µg/m³)`
                    ...
                    { // PM10 Hedef Değeri
                        label: `PM10 Hedef (%${targetPercentage})`,
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'PM10 Değerleri: Sınır, Hedef ve Kirlilik Kaynağı'
                    },
                    tooltip: {
                        callbacks: {
    ...
    calculateButton.addEventListener('click', () => {
        ...
        const reducedPM10 = pollutantData.pm10.map(value => value !== null ? value - (value * reductionPercentage) : 0);

        // Daha önce eklenen azaltılmış veri setini kaldır
        const datasetIndex = chart.data.datasets.findIndex(dataset => dataset.label.includes('PM10 Azaltılmış'));
        if (datasetIndex !== -1) {
        ...
        chart.data.datasets.push({
            label: `PM10 Azaltılmış (%${reductionPercentage * 100})`,
            data: reducedPM10,
            borderColor: 'blue',
            borderDash: [4, 4],
            borderWidth: 2,
